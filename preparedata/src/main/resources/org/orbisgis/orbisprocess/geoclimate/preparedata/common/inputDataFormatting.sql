...
CREATE TABLE $BU_ZONE (THE_GEOM geometry, ID_BUILD serial, ID_SOURCE varchar(24), HEIGHT_WALL integer, HEIGHT_ROOF integer, NB_LEV integer, TYPE varchar, MAIN_USE varchar, ZINDEX integer) AS SELECT ST_FORCE2D(THE_GEOM), null, ID_SOURCE, HEIGHT_WALL, HEIGHT_ROOF, NB_LEV, TYPE, MAIN_USE, ZINDEX FROM $INPUT_BUILDING;
...
CREATE INDEX ON $BUILDING_FC USING BTREE(ID_ZONE);
CREATE INDEX ON $BUILDING_FC USING BTREE(HEIGHT_WALL);
...
CREATE TABLE $BUILDING (THE_GEOM geometry, ID_BUILD integer PRIMARY KEY, ID_SOURCE varchar(24), HEIGHT_WALL integer, HEIGHT_ROOF integer, NB_LEV integer, TYPE varchar, MAIN_USE varchar, ZINDEX integer, ID_ZONE varchar)
   AS SELECT CASE WHEN ST_ISVALID(THE_GEOM) THEN ST_FORCE2D(ST_NORMALIZE(THE_GEOM)) ELSE ST_FORCE2D(ST_MAKEVALID(THE_GEOM)) END, ID_BUILD, ID_SOURCE, HEIGHT_WALL, HEIGHT_ROOF, NB_LEV, TYPE, MAIN_USE, CASE WHEN ZINDEX is null THEN 0 ELSE ZINDEX END, ID_ZONE FROM $BUILDING_FC;
...
CREATE INDEX ON $BUILDING USING RTREE(the_geom);
CREATE INDEX ON $BUILDING USING BTREE(ID_ZONE);
...
CREATE TABLE $BUILD_LEV_RANGE AS SELECT COUNT(*) as LEV_RANGE FROM $BUILDING WHERE NB_LEV < 0 OR NB_LEV > 200;
...
CREATE TABLE $BUILDING_STATS_EXT_ZONE AS SELECT a.ID_ZONE, b.NB_BUILD, (c.NOT_VALID + q.NOT_VALID) as NOT_VALID, d.IS_EMPTY, (e.IS_EQUALS + o.IS_EQUALS) as IS_EQUALS, (f.OVERLAP + p.OVERLAP) as OVERLAP, g.FC_H_ZERO, g.FC_H_NULL, g.FC_H_RANGE, h.H_NULL, i.H_RANGE, j.H_ROOF_MIN_WALL, k.LEV_NULL, l.LEV_RANGE, m.NO_TYPE, n.TYPE_RANGE FROM $ZONE a, $BUILD_NUMB b, $BUILD_VALID_ZONE c, $BUILD_EMPTY d, $BUILD_EQUALS_ZONE e, $BUILD_OVERLAP_ZONE f, $FC_BUILD_STATS_EXT_ZONE g, $BUILD_H h, $BUILD_H_RANGE i, $BUILD_H_WALL_ROOF j, $BUILD_LEV k, $BUILD_LEV_RANGE l, $BUILD_TYPE m, $BUILD_TYPE_RANGE n, $BUILD_EQUALS_EXT_ZONE o, $BUILD_OVERLAP_EXT_ZONE p, $BUILD_VALID_EXT_ZONE q;

DROP TABLE IF EXISTS $BUILD_NUMB, $BUILD_VALID_ZONE, $BUILD_VALID_EXT_ZONE, $BUILD_EMPTY, $BUILD_EQUALS_ZONE, $BUILD_OVERLAP_ZONE, $BUILD_EQUALS_EXT_ZONE, $BUILD_OVERLAP_EXT_ZONE, $FC_BUILD_STATS_EXT_ZONE, $BUILD_H, $BUILD_H_RANGE, $BUILD_H_WALL_ROOF, $BUILD_LEV, $BUILD_LEV_RANGE, $BUILD_TYPE, $BUILD_TYPE_RANGE;
...
CREATE TABLE $ROAD_FC_W_RANGE AS SELECT COUNT(a.*) as FC_W_RANGE FROM $INPUT_ROAD a, $ZONE b WHERE a.the_geom && b.the_geom AND ST_INTERSECTS(a.the_geom, b.the_geom) AND (a.WIDTH<0 OR a.WIDTH>100);
...
CREATE TABLE $ROAD_FC_W_RANGE AS SELECT COUNT(*) as FC_W_RANGE FROM $INPUT_ROAD WHERE WIDTH<0 OR WIDTH>100;
...
CREATE TABLE $ROAD (THE_GEOM geometry, ID_ROAD serial, ID_SOURCE varchar(24), WIDTH double precision, TYPE varchar, SURFACE varchar, SIDEWALK varchar, ZINDEX integer, CROSSING varchar)
    AS SELECT CASE WHEN ST_ISVALID(THE_GEOM) THEN ST_FORCE2D(THE_GEOM) ELSE ST_FORCE2D(ST_MAKEVALID(THE_GEOM)) END, null, ID_SOURCE, WIDTH, TYPE, SURFACE, SIDEWALK, CASE WHEN ZINDEX is null THEN 0 ELSE ZINDEX END, CROSSING FROM ST_EXPLODE('$INPUT_ROAD');
...
CREATE INDEX ON $ROAD USING RTREE(the_geom);
...
CREATE INDEX ON $ROAD_ZONE USING RTREE(the_geom);


DROP TABLE IF EXISTS $ROAD_NUMB, $ROAD_VALID, $ROAD_EMPTY, $ROAD_EQUALS, $ROAD_OVERLAP, $ROAD_W, $ROAD_W_RANGE, $ROAD_TYPE, $ROAD_TYPE_RANGE, $ROAD_STATS_ZONE;
-- Count the number of roads
CREATE TABLE $ROAD_NUMB AS SELECT COUNT(*) as NB_ROAD FROM $ROAD_ZONE;
-- Count the number of roads which geometry is not valid
CREATE TABLE $ROAD_VALID AS SELECT COUNT(*) as NOT_VALID FROM $ROAD_ZONE WHERE NOT ST_IsValid(the_geom);
-- Count the number of roads which geometry is empty
CREATE TABLE $ROAD_EMPTY AS SELECT COUNT(*) as IS_EMPTY FROM $ROAD_ZONE WHERE ST_IsEmpty(the_geom);
-- Count the number of roads which geometry is equal to an another one
...
-- Count the number of roads which overlaps another one
...
CREATE TABLE $ROAD_W_RANGE AS SELECT COUNT(*) as W_RANGE FROM $ROAD_ZONE WHERE WIDTH<0 OR WIDTH>100;
...
-- Merge all these information into one single table
CREATE TABLE $ROAD_STATS_ZONE AS SELECT a.ID_ZONE, b.NB_ROAD, c.NOT_VALID, d.IS_EMPTY, e.IS_EQUALS, f.OVERLAP, g.FC_W_ZERO, g.FC_W_NULL, g.FC_W_RANGE, j.W_NULL, k.W_RANGE, l.NO_TYPE, m.TYPE_RANGE FROM $ZONE a, $ROAD_NUMB b, $ROAD_VALID c, $ROAD_EMPTY d, $ROAD_EQUALS e, $ROAD_OVERLAP f, $R_FC_STATS_ZONE g, $ROAD_W j, $ROAD_W_RANGE k, $ROAD_TYPE l, $ROAD_TYPE_RANGE m;


-- Clean not needed layers
-----------------------
DROP TABLE IF EXISTS $ROAD_NUMB, $ROAD_VALID, $ROAD_EMPTY, $ROAD_EQUALS, $ROAD_OVERLAP, $ROAD_W, $ROAD_W_RANGE, $ROAD_TYPE, $ROAD_TYPE_RANGE, $R_FC_STATS_ZONE;
...
-- Count the number of roads
CREATE TABLE $ROAD_NUMB AS SELECT COUNT(*) as NB_ROAD FROM $ROAD;
...
CREATE TABLE $ROAD_W_RANGE AS SELECT COUNT(*) as W_RANGE FROM $ROAD WHERE WIDTH < 0 OR WIDTH > 100;
-- Count the number of vegetation areas with no TYPE or where TYPE is not in the list of tags (Table NATURE_TAGS)
...
-- Clean not needed layers
-----------------------
DROP TABLE IF EXISTS $ROAD_ZONE, $INPUT_ROAD, $ROAD_NUMB, $ROAD_VALID, $ROAD_EMPTY, $ROAD_EQUALS, $ROAD_OVERLAP, $ROAD_W, $ROAD_W_RANGE, $ROAD_TYPE, $ROAD_TYPE_RANGE, $R_FC_STATS_EXT_ZONE;
...
CREATE TABLE $RAIL(THE_GEOM geometry, ID_RAIL serial, ID_SOURCE varchar(24), TYPE varchar, ZINDEX integer, CROSSING varchar)
	AS SELECT CASE WHEN ST_ISVALID(THE_GEOM) THEN ST_FORCE2D(THE_GEOM) ELSE ST_FORCE2D(ST_MAKEVALID(THE_GEOM)) END, null, ID_SOURCE, TYPE, CASE WHEN ZINDEX is null THEN 0 ELSE ZINDEX END, CROSSING FROM ST_EXPLODE('$INPUT_RAIL');

-- Filling the CROSSING column with a 'null' value when no value
UPDATE $ROAD SET CROSSING = 'null' WHERE CROSSING is null;

CREATE INDEX ON $RAIL USING RTREE(the_geom);
...
CREATE TABLE $HYDRO (THE_GEOM geometry, ID_HYDRO serial, ID_SOURCE varchar(24))
	AS SELECT (CASE WHEN ST_ISVALID(THE_GEOM) THEN ST_FORCE2D(THE_GEOM) ELSE ST_FORCE2D(ST_MAKEVALID(THE_GEOM)) END), null, ID_SOURCE FROM ST_EXPLODE('$INPUT_HYDRO');
CREATE INDEX ON $HYDRO USING RTREE(the_geom);
...
DROP TABLE IF EXISTS $HYDRO_NB, $HYDRO_VALID, $HYDRO_EMPTY, $HYDRO_EQUALS, $HYDRO_OVERLAP, $HYDRO_STATS_EXT_ZONE;

CREATE TABLE $HYDRO_NB AS SELECT COUNT(*) as NB_HYDRO FROM $HYDRO;
...
CREATE TABLE $HYDRO_STATS_EXT_ZONE AS SELECT a.ID_ZONE, b.NB_HYDRO, c.NOT_VALID, d.IS_EMPTY, e.IS_EQUALS, f.OVERLAP FROM $ZONE a, $HYDRO_NB b, $HYDRO_VALID c, $HYDRO_EMPTY d, $HYDRO_EQUALS e, $HYDRO_OVERLAP f;


-- Clean not needed layers
-----------------------
DROP TABLE IF EXISTS $HYDRO_NB, $HYDRO_VALID, $HYDRO_EMPTY, $HYDRO_EQUALS, $HYDRO_OVERLAP;
...
CREATE TABLE $VEGET (THE_GEOM geometry, ID_VEGET serial, ID_SOURCE varchar(24), TYPE varchar, HEIGHT_CLASS varchar)
	AS SELECT (CASE WHEN ST_ISVALID(THE_GEOM) THEN ST_FORCE2D(THE_GEOM) ELSE ST_FORCE2D(ST_MAKEVALID(THE_GEOM)) END), null, ID_SOURCE, TYPE, null FROM ST_EXPLODE('$INPUT_VEGET');
CREATE INDEX ON $VEGET USING RTREE(the_geom);
...
DROP TABLE IF EXISTS $VEGET_NB, $VEGET_VALID, $VEGET_EMPTY, $VEGET_EQUALS, $VEGET_OVERLAP, $VEGET_TYPE, $VEGET_TYPE_RANGE, $VEGET_STATS_EXT_ZONE;

CREATE TABLE $VEGET_NB AS SELECT COUNT(*) as NB_VEGET FROM $VEGET;
...
CREATE TABLE $VEGET_STATS_EXT_ZONE AS SELECT a.ID_ZONE, b.NB_VEGET, c.NOT_VALID, d.IS_EMPTY, e.IS_EQUALS, f.OVERLAP, g.NO_TYPE, h.TYPE_RANGE FROM $ZONE a, $VEGET_NB b, $VEGET_VALID c, $VEGET_EMPTY d, $VEGET_EQUALS e, $VEGET_OVERLAP f, $VEGET_TYPE g, $VEGET_TYPE_RANGE h;


-- Clean not needed layers
-----------------------
DROP TABLE IF EXISTS $VEGET_NB, $VEGET_VALID, $VEGET_EMPTY, $VEGET_EQUALS, $VEGET_OVERLAP, $VEGET_TYPE, $VEGET_TYPE_RANGE;
...
CREATE TABLE $IMPERVIOUS (THE_GEOM geometry, ID_IMPERVIOUS serial, ID_SOURCE varchar(24))
	AS SELECT (CASE WHEN ST_ISVALID(THE_GEOM) THEN ST_FORCE2D(THE_GEOM) ELSE ST_FORCE2D(ST_MAKEVALID(THE_GEOM)) END), null, ID_SOURCE FROM ST_EXPLODE('$INPUT_IMPERVIOUS');
CREATE INDEX ON $IMPERVIOUS USING RTREE(the_geom);

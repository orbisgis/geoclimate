...
2. Block scale, defined as the union of the geometries of buildings touching each other (at least one point in common). Note that the union can represent a single building. 
...
GeoClimate output data consists in both a set of indicators and classifications. GeoClimate uses the concept of Workflow to chain a set of spatial analysis and statistical processes. The Workflow is organized in 3 steps (\autoref{fig:processing_steps}). Each step or each process within a step can be run individually. A Workflow can be used even if not all input data are provided: partitioning and indicators calculations will then be limited to the supplied data.

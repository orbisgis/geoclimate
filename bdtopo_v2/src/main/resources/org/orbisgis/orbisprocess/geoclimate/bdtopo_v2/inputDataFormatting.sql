...
CREATE TABLE $RAIL(THE_GEOM geometry, ID_RAIL serial, ID_SOURCE varchar(24), TYPE varchar, ZINDEX integer, CROSSING varchar)
	AS SELECT ST_FORCE2D(THE_GEOM) as the_geom, CAST((row_number() over()) as Integer), ID_SOURCE, TYPE, CASE WHEN ZINDEX is null THEN 0 ELSE ZINDEX END, CROSSING FROM ST_EXPLODE('$INPUT_RAIL');
...
CREATE TABLE $HYDRO (THE_GEOM geometry, ID_HYDRO serial, ID_SOURCE varchar(24))
	AS SELECT ST_FORCE2D(THE_GEOM) as the_geom, CAST((row_number() over()) as Integer), ID_SOURCE FROM ST_EXPLODE('$INPUT_HYDRO');
...
CREATE TABLE $VEGET (THE_GEOM geometry, ID_VEGET serial, ID_SOURCE varchar(24), TYPE varchar, HEIGHT_CLASS varchar)
	AS SELECT ST_FORCE2D(THE_GEOM) as the_geom, CAST((row_number() over()) as Integer), ID_SOURCE, TYPE, null FROM ST_EXPLODE('$INPUT_VEGET');
...
CREATE TABLE $IMPERVIOUS (THE_GEOM geometry, ID_IMPERVIOUS serial, ID_SOURCE varchar(24))
	AS SELECT ST_FORCE2D(THE_GEOM) as the_geom, CAST((row_number() over()) as Integer), ID_SOURCE FROM ST_EXPLODE('$INPUT_IMPERVIOUS');

...
CREATE TABLE $BUILDING (THE_GEOM geometry, ID_BUILD integer PRIMARY KEY, ID_SOURCE varchar(24), HEIGHT_WALL integer, HEIGHT_ROOF integer, NB_LEV integer, TYPE varchar, MAIN_USE varchar, ZINDEX integer, ID_ZONE varchar)
   AS SELECT ST_FORCE2D(THE_GEOM) as the_geom, ID_BUILD, ID_SOURCE, HEIGHT_WALL, HEIGHT_ROOF, NB_LEV, TYPE, MAIN_USE, CASE WHEN ZINDEX is null THEN 0 ELSE ZINDEX END, ID_ZONE FROM $BUILDING_FC;
...
CREATE TABLE $ROAD (THE_GEOM geometry, ID_ROAD serial, ID_SOURCE varchar(24), WIDTH double precision, TYPE varchar, SURFACE varchar, SIDEWALK varchar, ZINDEX integer, CROSSING varchar)
    AS SELECT ST_FORCE2D(THE_GEOM) as the_geom, null, ID_SOURCE, WIDTH, TYPE, SURFACE, SIDEWALK, CASE WHEN ZINDEX is null THEN 0 ELSE ZINDEX END, CROSSING FROM ST_EXPLODE('$INPUT_ROAD');
...
CREATE TABLE $RAIL(THE_GEOM geometry, ID_RAIL serial, ID_SOURCE varchar(24), TYPE varchar, ZINDEX integer, CROSSING varchar)
	AS SELECT ST_FORCE2D(THE_GEOM) as the_geom, null, ID_SOURCE, TYPE, CASE WHEN ZINDEX is null THEN 0 ELSE ZINDEX END, CROSSING FROM ST_EXPLODE('$INPUT_RAIL');
...
CREATE TABLE $HYDRO (THE_GEOM geometry, ID_HYDRO serial, ID_SOURCE varchar(24))
	AS SELECT ST_FORCE2D(THE_GEOM) as the_geom, null, ID_SOURCE FROM ST_EXPLODE('$INPUT_HYDRO');
...
CREATE TABLE $VEGET (THE_GEOM geometry, ID_VEGET serial, ID_SOURCE varchar(24), TYPE varchar, HEIGHT_CLASS varchar)
	AS SELECT ST_FORCE2D(THE_GEOM) as the_geom, null, ID_SOURCE, TYPE, null FROM ST_EXPLODE('$INPUT_VEGET');
...
CREATE TABLE $IMPERVIOUS (THE_GEOM geometry, ID_IMPERVIOUS serial, ID_SOURCE varchar(24))
	AS SELECT ST_FORCE2D(THE_GEOM) as the_geom, null, ID_SOURCE FROM ST_EXPLODE('$INPUT_IMPERVIOUS');

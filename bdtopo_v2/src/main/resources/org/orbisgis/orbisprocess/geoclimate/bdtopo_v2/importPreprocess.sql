...
CREATE INDEX IF NOT EXISTS idx_geom_PISTE_AERODROME ON $PISTE_AERODROME USING RTREE(the_geom);
CREATE INDEX IF NOT EXISTS idx_geom_$RESERVOIR ON $RESERVOIR USING RTREE(the_geom);
...
CREATE TABLE $BU_ZONE_RESERVOIR (THE_GEOM geometry, ID_SOURCE varchar(24), HEIGHT_WALL integer, TYPE varchar)
    AS SELECT CASE WHEN ST_ISVALID(a.THE_GEOM) THEN ST_FORCE2D(ST_NORMALIZE(a.THE_GEOM)) ELSE ST_FORCE2D(ST_MAKEVALID(a.THE_GEOM)) END, a.ID, a.HAUTEUR, a.NATURE FROM $RESERVOIR a, $ZONE_BUFFER b WHERE a.the_geom && b.the_geom AND ST_INTERSECTS(a.the_geom, b.the_geom) and a.NATURE='Réservoir industriel' and a.HAUTEUR>0;
...
UNION SELECT THE_GEOM, ID_SOURCE, HEIGHT_WALL, HEIGHT_WALL, 0, 'heavy_industry', 'industrial', 0 FROM ST_EXPLODE('$BU_ZONE_RESERVOIR');
...
CREATE TABLE $INPUT_IMPERVIOUS (THE_GEOM geometry, ID_SOURCE varchar(24))
 ...
 UNION SELECT THE_GEOM, ID_SOURCE FROM ST_EXPLODE('$TMP_IMPERV_SURFACE_ACTIVITE')
  UNION SELECT THE_GEOM, ID_SOURCE FROM ST_EXPLODE('$TMP_IMPERV_PISTE_AERODROME');

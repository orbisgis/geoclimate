...
abstract class ProcessingChains extends Script {
     ...
     * @param inputIdUp The id of the RSU table (default "id_rsu")
    ...
    static IProcess createLCZ(){
        return processFactory.create("Merge the geometries that touch each other",
                ...
                { datasource, prefixName, buildingTable, rsuTable, roadTable, vegetationTable,
                  ...
                  heightColumnName = "height_roof", inputIdUp = "id_rsu", fractionTypePervious = ["low"],
                  fractionTypeImpervious = ["road"], inputFields = ["id_build", "the_geom"], levelForRoads = [0] ->
                    ...
                    if(fractionTypePervious.contains("low_vegetation") | fractionTypePervious.contains("high_vegetation")){
                        if(fractionTypePervious.contains("low_vegetation") & fractionTypePervious.contains("high_vegetation")){
                            veg_type.plus("all")
                            perv_type.plus("all_vegetation_fraction")
                        }
                        else if(fractionTypePervious.contains("low_vegetation")){
                            veg_type.plus("low")
                            perv_type.plus("low_vegetation_fraction")
                        }
                        else if(fractionTypePervious.contains("high_vegetation")){
                            veg_type.plus("high")
                            perv_type.plus("high_vegetation_fraction")
                        }
                        ...
                        // Add the vegetation fields in the surface fraction tables used for pervious and impervious fractions
                        veg_type.each{indic ->
                            datasource.execute("ALTER TABLE $rsu_surf_fractions ADD COLUMN $indic DOUBLE; " +
                                    "UPDATE $rsu_surf_fractions SET $indic = (SELECT b.$indic FROM $rsu_surf_fractions a " +
                                    "INNER JOIN ${calc_veg_frac.results.outputTableName} b ON a.$columnIdRsu = b.$columnIdRsu")
                        }
                    ...
                    if(fractionTypeImpervious.contains("road")){
                        imp_type.plus("ground_road_fraction")
                        ...
                        datasource.execute("ALTER TABLE $rsu_surf_fractions ADD COLUMN ground_road_fraction DOUBLE; " +
                                ...
                                " FROM $rsu_surf_fractions a INNER JOIN ${calc_road_frac.results.outputTableName} b " +
                                "ON a.$columnIdRsu = b.$columnIdRsu")
                    }
                    ...
                    if(fractionTypePervious.contains("water")){
                        ...
                        datasource.execute("ALTER TABLE $rsu_surf_fractions ADD COLUMN water_fraction DOUBLE; " +
                                ...
                                " FROM $rsu_surf_fractions a INNER JOIN ${calc_water_frac.results.outputTableName} b " +
                                "ON a.$columnIdRsu = b.$columnIdRsu")
                    }
                    ...
                    calc_proj_facade_dist.execute([buildingTable: "tempo_build", rsuTable: "rsu_test",
                    ...
                    calc_build_densityNroughness.execute([inputLowerScaleTableName: calc_building_area.results.outputTableName,
                                                          inputUpperScaleTableName: rsuTable, inputIdUp: inputIdUp,
                    ...
                    datasource.execute("DROP TABLE IF EXISTS $rsu_indic0; CREATE TABLE $rsu_indic0 AS SELECT a.*, " +
                            "b.dens_building_area, b.geom_avg_$heightColumnName FROM $rsuTable a INNER JOIN " +
                            "${calc_build_densityNroughness.results.outputTableName} b ON a.$columnIdRsu = b.$columnIdRsu")
                    ...
                    calcSVF.execute([rsuTable: rsu_indic0, correlationBuildingTable: buildingTable, rsuBuildingDensityColumn:
                            "dens_building_area", svfPointDensity: svfPointDensity, svfRayLength: svfRayLength,
                                     svfNumberOfDirection: svfNumberOfDirection, prefixName: prefixName, datasource: datasource])
                    ...
                    datasource.execute("DROP TABLE IF EXISTS $rsu_4_aspectratio; CREATE TABLE $rsu_4_aspectratio AS SELECT a.*, " +
                            ...
                            "${calc_free_ext_density.results.outputTableName} b ON a.$columnIdRsu = b.$columnIdRsu")
                    IProcess  calc_aspect_ratio =  Geoclimate.RsuIndicators.aspectRatio()
                    calc_aspect_ratio.execute([rsuTable: rsuTable, rsuFreeExternalFacadeDensityColumn:
                            "rsu_free_external_facade_density", rsuBuildingDensityColumn: "dens_building_area",
                                               prefixName: prefixName, datasource: datasource])
                    ...
                    if(fractionTypeImpervious.contains("building")) {
                        imp_type.plus("dens_building_area")
                        datasource.execute("ALTER TABLE $rsu_surf_fractions ADD COLUMN dens_building_area DOUBLE; " +
                                "UPDATE $rsu_surf_fractions SET dens_building_area = (SELECT b.dens_building_area" +
                                " FROM $rsu_surf_fractions a INNER JOIN ${calc_build_densityNroughness.results.outputTableName} b " +
                                "ON a.$columnIdRsu = b.$columnIdRsu")
                    }
                    ...
                    datasource.execute("DROP TABLE IF EXISTS $rsu_4_roughness1; CREATE TABLE $rsu_4_roughness1 AS SELECT b.*, " +
                            ...
                            "${calc_proj_facade_dist.results.outputTableName} b ON a.$columnIdRsu = b.$columnIdRsu")
                    ...
                    datasource.execute("DROP TABLE IF EXISTS $rsu_indic1; CREATE TABLE $rsu_indic1 AS SELECT a.*, " +
                            ...
                            "${calc_aspect_ratio.results.outputTableName} b ON a.$columnIdRsu = b.$columnIdRsu")
                    datasource.execute("DROP TABLE IF EXISTS $rsu_indic2; CREATE TABLE $rsu_indic2 AS SELECT a.*, " +
                            ...
                            "${calcSVF.results.outputTableName} b ON a.$columnIdRsu = b.$columnIdRsu")
                    datasource.execute("DROP TABLE IF EXISTS $rsu_indic3; CREATE TABLE $rsu_indic3 AS SELECT a.*, " +
                            ...
                            "${calc_perviousness_frac.results.outputTableName} b ON a.$columnIdRsu = b.$columnIdRsu")
                    datasource.execute("DROP TABLE IF EXISTS $rsu_all_indic; CREATE TABLE $rsu_all_indic AS SELECT a.*, " +
                            ...
                            "${calc_roughness_class.results.outputTableName} b ON a.$columnIdRsu = b.$columnIdRsu")

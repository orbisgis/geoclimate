...
class InputDataLoadingV2Test {
    ...
    H2GIS createH2GIS(String dbPath){
        ...
        H2GIS h2GISDatabase = H2GIS.open(folder.getAbsolutePath()+File.separator+dbPath+";AUTO_SERVER=TRUE")
    ...
    @Test
    void prepareFormatTest() {
        def h2GISDatabase =  createH2GIS("prepareFormatBDTopov2_2Test")

        //Prepare the data
        def process = BDTopo.InputDataLoading.prepareBDTopoData()
        ...
        assertEquals('DOUBLE PRECISION', table.columnType('HEIGHT_WALL'))
        assertEquals('DOUBLE PRECISION', table.columnType('HEIGHT_ROOF'))

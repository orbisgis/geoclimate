    ...
    processFactory.create("Prepare the abstract model to build the RSU",
            ...
            { zoneTable ,roadTable, railTable, vegetationTable , hydrographicTable, surface_vegetation =100000, surface_hydrographic=2500 ,outputTableName, datasource ->
                ...
                if(numberZone==1){
                    ...
                    String  vegetation_indice = vegetationTable+uuid_tmp
                    datasource.execute "DROP TABLE IF EXISTS $vegetation_indice".toString()
                    datasource.execute "CREATE TABLE $vegetation_indice(THE_GEOM geometry, ID serial, CONTACT integer) AS (SELECT THE_GEOM, null , 0 FROM ST_EXPLODE('(SELECT * FROM $vegetationTable)') " +
                    ...
                    datasource.execute " UPDATE $vegetation_indice SET CONTACT=1 WHERE ID IN(SELECT DISTINCT(a.ID) FROM $vegetation_indice a, $vegetation_indice b WHERE a.THE_GEOM && b.THE_GEOM AND ST_INTERSECTS(a.THE_GEOM, b.THE_GEOM) AND a.ID<>b.ID)"
                    ...
                    datasource.execute "CREATE TABLE $vegetation_unified AS " +
                            "(SELECT the_geom FROM ST_EXPLODE('(SELECT ST_UNION(ST_ACCUM(THE_GEOM)) AS THE_GEOM FROM $vegetation_indice WHERE CONTACT=1)') " +
                            "where st_dimension(the_geom)>0 AND st_isempty(the_geom)=false AND st_area(the_geom)> $surface_vegetation) " +
                            "UNION ALL (SELECT THE_GEOM FROM $vegetation_indice WHERE contact=0 AND st_area(the_geom)> $surface_vegetation)"

                    datasource.execute "CREATE  INDEX IF NOT EXISTS veg_unified_idx ON  $vegetation_unified(THE_GEOM) using rtree"
                    ...
                    datasource.execute "CREATE TABLE $road_tmp AS SELECT a.the_geom AS THE_GEOM FROM $roadTable where zindex=0".toString()
                    ...
                    datasource.execute "CREATE TABLE $rail_tmp AS SELECT a.the_geom AS THE_GEOM FROM $railTable where zindex=0".toString()

                    // The input table that contains the geometries to be transformed as RSU
                    logger.info("Grouping all tables...")
                    datasource.execute "DROP TABLE if exists $outputTableName"
                    datasource.execute "CREATE TABLE $outputTableName AS (SELECT THE_GEOM FROM $road_tmp) UNION (SELECT THE_GEOM FROM $rail_tmp) " +
                            "UNION (SELECT THE_GEOM FROM $hydrographic_tmp) UNION  (SELECT THE_GEOM FROM $vegetation_tmp)"

                    [outputTableName: unified_geometries]
                }

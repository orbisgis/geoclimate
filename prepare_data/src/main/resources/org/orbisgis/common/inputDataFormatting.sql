...
-- Comments:																					--
--   - STEP 1 has to be executed only one times (the first time)								--
--   - STEP 2 has to be executed for each city you want to study 								--
--																								--
-- -*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/-*/--
...
---------------------------------------------------------------------------------
-- Declaration of variables, to be used in this script
---------------------------------------------------------------------------------

-- The unique ID of the commune (in France, the INSEE CODE)
SET @ID_ZONE=56260;
-- Defines min/max height (in meters) paramaters for one level
SET @H_LEV_MIN=3;
SET @H_LEV_MAX=15;
-- Defines the height threshold (in meters) at which the rule is applied
SET @H_THRESHOLD_LEV2 = 10;
...
DROP TABLE IF EXISTS $BUILD_NUMB, $BUILD_VALID_ZONE, $BUILD_EMPTY, $BUILD_EQUALS_ZONE, $BUILD_OVERLAP_ZONE, $BUILD_H, $BUILD_H_RANGE, $BUILD_H_WALL_ROOF, $BUILD_LEV, $BUILD_LEV_RANGE, $BUILD_TYPE, $BUILD_TYPE_RANGE, $B_STATS_ZONE;
...
CREATE TABLE $B_STATS_ZONE AS SELECT a.ID_ZONE, b.NB_BUILD, c.NOT_VALID, d.IS_EMPTY, e.IS_EQUALS, f.OVERLAP, g.FC_H_ZERO, g.FC_H_NULL, g.FC_H_RANGE, h.H_NULL, i.H_RANGE, j.H_ROOF_MIN_WALL, k.LEV_NULL, l.LEV_RANGE, m.NO_TYPE, n.TYPE_RANGE FROM $ZONE a, $BUILD_NUMB b, $BUILD_VALID_ZONE c, $BUILD_EMPTY d, $BUILD_EQUALS_ZONE e, $BUILD_OVERLAP_ZONE f, $FC_BUILD_STATS_ZONE g, $BUILD_H h, $BUILD_H_RANGE i, $BUILD_H_WALL_ROOF j, $BUILD_LEV k, $BUILD_LEV_RANGE l, $BUILD_TYPE m, $BUILD_TYPE_RANGE n;

-- Feeding the national table
-- INSERT INTO BUILDING_STATS_ZONE SELECT ID_ZONE, NB_BUILD, NOT_VALID, IS_EMPTY, IS_EQUALS, OVERLAP, FC_H_ZERO, FC_H_NULL, FC_H_RANGE, H_NULL, H_RANGE, H_ROOF_MIN_WALL, LEV_NULL, LEV_RANGE, NO_TYPE, TYPE_RANGE FROM $B_STATS_ZONE;

-- Clean not needed layers
DROP TABLE IF EXISTS $BUILD_NUMB, $BUILD_EMPTY, $FC_BUILD_STATS_ZONE, $BUILD_H, $BUILD_H_RANGE, $BUILD_H_WALL_ROOF, $BUILD_LEV, $BUILD_LEV_RANGE, $BUILD_TYPE, $BUILD_TYPE_RANGE, $B_STATS_ZONE;


-- 2. Analyze at the city buffer level
-----------------------

DROP TABLE IF EXISTS $BUILD_VALID_EXT_ZONE, $BUILD_EMPTY, $BUILD_EQUALS_EXT_ZONE, $BUILD_OVERLAP_EXT_ZONE, $BUILD_H, $BUILD_H_RANGE, $BUILD_H_WALL_ROOF, $BUILD_LEV, $BUILD_LEV_RANGE, $BUILD_TYPE, $BUILD_TYPE_RANGE, $B_STATS_EXT_ZONE;
...
DROP TABLE IF EXISTS $ROAD_FC_W_ZERO, $ROAD_FC_W_NULL, $ROAD_FC_W_RANGE, $R_FC_STATS_ZONE_BUF;
...
CREATE TABLE $R_FC_STATS_ZONE_BUF AS SELECT a.ID_ZONE, b.FC_W_ZERO, c.FC_W_NULL, d.FC_W_RANGE FROM $ZONE a, $ROAD_FC_W_ZERO b, $ROAD_FC_W_NULL c, $ROAD_FC_W_RANGE d;
...
DROP TABLE IF EXISTS $ROAD_VALID, $ROAD_EMPTY, $ROAD_EQUALS, $ROAD_OVERLAP, $ROAD_W, $ROAD_W_RANGE, $ROAD_TYPE, $ROAD_TYPE_RANGE, R_STATS;
...
-- Merge all these informations into one single table
CREATE TABLE R_STATS AS SELECT a.ID_ZONE, COUNT(b.*) as NB_ROAD, c.NOT_VALID, d.IS_EMPTY, e.IS_EQUALS, f.OVERLAP, g.FC_W_ZERO, g.FC_W_NULL, g.FC_W_RANGE, j.W_NULL, k.W_RANGE, l.NO_TYPE, m.TYPE_RANGE FROM $ZONE a, $ROAD_ZONE b, $ROAD_VALID c, $ROAD_EMPTY d, $ROAD_EQUALS e, $ROAD_OVERLAP f, $R_FC_STATS_ZONE g, $ROAD_W j, $ROAD_W_RANGE k, $ROAD_TYPE l, $ROAD_TYPE_RANGE m;


-- Feeding the national table
-----------------------
--INSERT INTO ROAD_STATS_ZONE SELECT ID_ZONE, NB_ROAD, NOT_VALID, IS_EMPTY, IS_EQUALS, OVERLAP, FC_W_ZERO, FC_W_NULL, FC_W_RANGE, W_NULL, W_RANGE, NO_TYPE, TYPE_RANGE FROM R_STATS;

-- Clean not needed layers
-----------------------
DROP TABLE IF EXISTS $ROAD_VALID, $ROAD_EMPTY, $ROAD_EQUALS, $ROAD_OVERLAP, $ROAD_W, $ROAD_W_RANGE, $ROAD_TYPE, $ROAD_TYPE_RANGE, R_STATS, $R_FC_STATS_ZONE;
...
-- Merge all these informations into one single table
CREATE TABLE R_STATS AS SELECT a.ID_ZONE, COUNT(b.*) as NB_ROAD, c.NOT_VALID, d.IS_EMPTY, e.IS_EQUALS, f.OVERLAP, g.FC_W_ZERO, g.FC_W_NULL, g.FC_W_RANGE, j.W_NULL, k.W_RANGE, l.NO_TYPE, m.TYPE_RANGE FROM $ZONE a, $ROAD b, $ROAD_VALID c, $ROAD_EMPTY d, $ROAD_EQUALS e, $ROAD_OVERLAP f, $R_FC_STATS_ZONE_BUF g, $ROAD_W j, $ROAD_W_RANGE k, $ROAD_TYPE l, $ROAD_TYPE_RANGE m;


-- Feeding the national table
-----------------------
--INSERT INTO ROAD_STATS_ZONE_BUF SELECT ID_ZONE, NB_ROAD, NOT_VALID, IS_EMPTY, IS_EQUALS, OVERLAP, FC_W_ZERO, FC_W_NULL, FC_W_RANGE, W_NULL, W_RANGE, NO_TYPE, TYPE_RANGE FROM R_STATS;

-- Clean not needed layers
-----------------------
DROP TABLE IF EXISTS $ROAD_ZONE, INPUT_ROAD, $ROAD_VALID, $ROAD_EMPTY, $ROAD_EQUALS, $ROAD_OVERLAP, $ROAD_W, $ROAD_W_RANGE, $ROAD_TYPE, $ROAD_TYPE_RANGE, R_STATS, $R_FC_STATS_ZONE_BUF;
...
-- Feeding the national table
-----------------------
--INSERT INTO RAIL_STATS_ZONE SELECT ID_ZONE, NB_RAIL, NOT_VALID, IS_EMPTY, IS_EQUALS, OVERLAP, NO_TYPE, TYPE_RANGE FROM R_STATS;
...
-- Feeding the national table
-----------------------
--INSERT INTO HYDRO_STATS_ZONE SELECT ID_ZONE, NB_HYDRO, NOT_VALID, IS_EMPTY, IS_EQUALS, OVERLAP FROM $H_STATS;
...
-- Feeding the national table
-----------------------
--INSERT INTO HYDRO_STATS_EXT_ZONE SELECT ID_ZONE, NB_HYDRO, NOT_VALID, IS_EMPTY, IS_EQUALS, OVERLAP FROM $H_STATS;
...
DROP TABLE IF EXISTS $VEGET_NUM_ZONE, $VEGET_VALID, $VEGET_EMPTY, $VEGET_EQUALS, VEGET_OVERLAP, VEGET_TYPE, VEGET_TYPE_RANGE, V_STATS;
...
-- Merge all these informations into one single table
CREATE TABLE V_STATS AS SELECT a.ID_ZONE, b.NB_VEGET, c.NOT_VALID, d.IS_EMPTY, e.IS_EQUALS, f.OVERLAP, g.NO_TYPE, h.TYPE_RANGE FROM $ZONE a, $VEGET_NUM_ZONE b, $VEGET_VALID c, $VEGET_EMPTY d, $VEGET_EQUALS e, VEGET_OVERLAP f, VEGET_TYPE g, VEGET_TYPE_RANGE h;

-- Feeding the national table
-----------------------
INSERT INTO VEGET_STATS_ZONE SELECT ID_ZONE, NB_VEGET, NOT_VALID, IS_EMPTY, IS_EQUALS, OVERLAP, NO_TYPE, TYPE_RANGE FROM V_STATS;
...
DROP TABLE IF EXISTS $VEGET_VALID, $VEGET_EMPTY, $VEGET_EQUALS, VEGET_OVERLAP, VEGET_TYPE, VEGET_TYPE_RANGE, V_STATS;
...
CREATE TABLE V_STATS AS SELECT a.ID_ZONE, COUNT(b.*) as NB_VEGET, c.NOT_VALID, d.IS_EMPTY, e.IS_EQUALS, f.OVERLAP, g.NO_TYPE, h.TYPE_RANGE FROM $ZONE a, $VEGET b, $VEGET_VALID c, $VEGET_EMPTY d, $VEGET_EQUALS e, VEGET_OVERLAP f, VEGET_TYPE g, VEGET_TYPE_RANGE h;

-- Feeding the national table
-----------------------
INSERT INTO VEGET_STATS_EXT_ZONE SELECT ID_ZONE, NB_VEGET, NOT_VALID, IS_EMPTY, IS_EQUALS, OVERLAP, NO_TYPE, TYPE_RANGE FROM V_STATS;
